...
//
// V4 stops passing directories around and starts fetching them.
//
// Under V3 a directory arrived as a side effect: every trade, catalogue read and
// delivery carried up to thirty-two principals in each direction, and `announce`
// existed to push one principal into a peer's table. All of it is gone. A
// designer now enters this table because the owner typed them in, because they
// proposed a trade, or because an owner-triggered crawl read them out of a
// peer's directory. `#exchange` and `#announce` have no successor for that
// reason, and `announced` has nothing left to record.
//
// `retired` is what a designer who uninstalled Chipswap leaves behind. It reads
// as ignored everywhere it matters, but it is a conclusion this canister drew
// rather than an instruction the owner gave, so it is a separate field: the
// owner may overrule it, and an inbound proposal disproves it outright.
// `strikes` is the evidence behind that conclusion, and it counts only calls a
// live peer would have answered.
...
module {
    ...
    // How this designer got here. There is no variant for a directory that
    // arrived unasked, because none can any more.
