...
//
// V6 stops storing other designers' work. The peer catalog cache held whole
// published catalogs — title, palette, and 757 pixel indices per design — for
// up to a hundred designers, inside a canister whose purpose is this owner's
// own chips. The browser now reads catalogs directly from the peers that
// publish them and keeps them on the machine that asked, where their age is
// visible and their storage is not the owner's to pay for.
//
...
// removed would drift silently. The directory keeps what it actually knows:
// who this designer is, how we met them, whether they answer, and whether the
// owner wants to hear from them.
//
...
module {
    ...
    // crawl reads those fields to decide who is overdue. Zero puts the seed at
    // the front of that queue, which is where a designer nobody has ever
    // contacted belongs.
