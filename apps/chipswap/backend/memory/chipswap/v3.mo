...
//
// V3 makes a directory entry ignorable and drops the auto-announce setting.
//
// Ignoring is deliberately not forgetting. A forgotten designer comes straight
// back the next time a peer shares their directory with us, and comes back with
// no memory of having been turned away; an ignored one stays in the table
// saying so. That is why `ignored` lives beside `announced` rather than being
// expressed as the absence of a row.
//
// Announcing is a decision the owner makes one peer at a time again, so the
// V2 `settings` root, whose only field enabled announcing in bulk, is gone.
...
module {
    ...
    // from them. The entry survives so that a peer re-sharing them does not
