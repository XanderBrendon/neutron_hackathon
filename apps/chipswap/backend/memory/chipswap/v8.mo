...
//
// V8 stops concluding that a designer is gone. `retired` was that conclusion,
// drawn from three consecutive paid calls a peer failed to answer, and
// `strikes` was the evidence it accumulated. Both leave.
//
// The reason is that the conclusion was never this canister's to draw well. The
// kernel does not say why a call was rejected, so the flag guessed from silence
// and was wrong in both directions: it retired canisters that were briefly
// stopped, and it stayed clear for canisters long gone that nobody had tried to
// trade with. Meanwhile the thing that actually notices a dead designer — the
// browser reading catalogs — was forbidden from touching the flag at all,
// because a peer who has merely not upgraded refuses a query too.
//
// So the judgement moves to the owner, who has the one piece of evidence this
// canister never had: what happened, in front of them, when the Market last
// tried. What is left here is `ignored`, which was always theirs.
//
// Every other type is V7's, unchanged.
